...
create visually appealing and effective compositions.
  ...
  // ============================================================
  // Phase 8: CMYK 印刷物パターン — document座標系の自動検出を検証
  // ============================================================
  printPhase(8, 'CMYK Print');

  const CMYK_W = 595; // A4幅 (≒210mm)
  const CMYK_H = 842; // A4高さ (≒297mm)
  ...
  await test('get_document_info → CMYK + document coordinate system', async () => {
    ...
    assert(
      ...
      `CMYK doc should recommend document coords, got ${info.workflowHint?.recommendedCoordinateSystem}`,
    );
  });
  ...
  await test('get_artboards → coordinateSystem is document', async () => {
    ...
    assert(ab.artboards.length >= 1, 'should have at least 1 artboard');
  });

  // document座標: Y-up, origin at bottom-left
  // create_rectangle position = top-left corner of rect
  // y=700 → 700pt up from bottom (near top of A4)
  const cmykRectX = 50;
  const cmykRectY = 700;
  const cmykRectW = 200;
  const cmykRectH = 100;
  ...
  await test('create_rectangle (document coords, CMYK fill)', async () => {
    ...
    assert(typeof r.uuid === 'string', 'should return uuid');
    assert(r.coordinateSystem === 'document', `coordinateSystem should be document, got ${r.coordinateSystem}`);
    cmykRectUuid = r.uuid;
  });

  await test('verify CMYK rect position via find_objects (document coords)', async () => {
  ...
  await test('modify_object position (document coords) + verify', async () => {
    ...
    const r = await callTool(client, 'modify_object', {
      uuid: cmykRectUuid,
      properties: { position: { x: newX, y: newY } },
    }) as any;
    assert(r.success === true, 'should succeed');
    assert(r.coordinateSystem === 'document', `coordinateSystem should be document, got ${r.coordinateSystem}`);

    const found = await callTool(client, 'find_objects', { name: '__e2e_cmyk_rect' }) as any;
    assertClose(found.objects[0].bounds.x, newX, 'moved x');
    assertClose(found.objects[0].bounds.y, newY, 'moved y');
  });
  ...
  await test('create_crop_marks → bleed note includes document coordinate system', async () => {
    const r = await callTool(client, 'create_crop_marks', { style: 'japanese' }) as any;
    assert(r.success === true, 'crop marks should succeed');
    ...
    assert(
      ...
      'bleed_required should mention document coordinate system',
    );
  });

  await test('set_workflow → explicit artboard-web override on CMYK doc', async () => {
    ...
    const ab = await callTool(client, 'get_artboards') as any;
    assert(ab.coordinateSystem === 'artboard-web', `overridden to artboard-web, got ${ab.coordinateSystem}`);

    // clear して auto-detect に戻す
    await callTool(client, 'set_workflow', { clear: true });
    const ab2 = await callTool(client, 'get_artboards') as any;
    assert(ab2.coordinateSystem === 'document', `should revert to document, got ${ab2.coordinateSystem}`);
  });
