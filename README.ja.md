...
Adobe は **Illustrator Beta**（30.4 以降、2026 年 8 月時点でも Beta 限定）に MCP サーバーを内蔵しています。既存ドキュメントの分析、一括リカラー、テンプレートからのバリエーション生成、複数アートボードの一括書き出し、フォント / リンク切れチェックなど既存ドキュメントの処理に強く、3 段階の権限管理（無確認実行 / 実行前確認 / 禁止）も内蔵しています。

既存ドキュメントの処理 — 分析、一括リカラー、テンプレートからのバリエーション生成、複数アートボードの一括書き出し、フォント / リンク切れチェック — は両者ともカバーしています。本プロジェクトの強みは、公式サーバーが未対応の**ゼロからの制作と印刷入稿ワークフロー**です。
